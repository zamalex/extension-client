      ...
      child: ListView(
        ...
        children: _buildButtonsRow()
            .map((Widget c) => Container(
                  child: c,
                  padding: widget.buttonPadding ?? const EdgeInsetsDirectional.only(end: kPaddingS),
                ))
            .toList(),
      ),
    );
  ...
  List<Widget> _buildButtonsRow() {
    return widget.buttonValues.map((ButtonGroupModel button) {
      ...
      return Card(
        ...
        color: _currentSelectedButton == widget.buttonValues[index] ? kPrimaryColor : Theme.of(context).highlightColor,
        shape: OutlineInputBorder(
          borderSide: BorderSide(
            color: _currentSelectedButton == widget.buttonValues[index] ? kPrimaryColor : Theme.of(context).highlightColor,
            width: 0,
          ),
          borderRadius: const BorderRadius.all(Radius.circular(kRoundedButtonRadius)),
        ),
        child: MaterialButton(
            onPressed: () {
              ...
              setState(() {
                if (widget.isUnselectable && _currentSelectedButton == widget.buttonValues[index]) {
            ...
            child: Center(
              child: StrutText(
                ...
                style: const TextStyle().copyWith(
                  color: _currentSelectedButton == widget.buttonValues[index] ? kWhite : Theme.of(context).textTheme.button.color,
                  fontWeight: _currentSelectedButton == widget.buttonValues[index] ? FontWeight.bold : FontWeight.w400,
                ),
              ),
            )),
      );
