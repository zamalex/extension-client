...
class _PickerState extends State<Picker> {
  ...
  void initState() {
    ...
    _items = widget.params['items'] as List<PickerItem<dynamic>> ?? <PickerItem<dynamic>>[];
    _itemsBackup = _items;
    _itemsSelected = widget.params['itemsSelected'] as List<PickerItem<dynamic>> ?? <PickerItem<dynamic>>[];
  ...
  void _onFilter(String text) {
    ...
    setState(() {
      _items = _items.where((PickerItem<dynamic> item) => item.text.toUpperCase().contains(text.toUpperCase())).toList();
    });
  }
  ...
  Future<void> _onSubmit() async {
    ...
    if (_itemsSelected.isNotEmpty) {
      if (_isMultiselection) {
        Navigator.pop(context, _itemsSelected.map<dynamic>((PickerItem<dynamic> item) => item.value as dynamic).toList());
      } else {
  ...
  Widget _buildList() {
    return ListView.builder(
      ...
      itemBuilder: (BuildContext context, int index) {
        ...
        final Widget trailing = _itemsSelected.contains(item) ? Icon(Icons.check, color: Theme.of(context).accentColor) : null;
        return ListItem(
          title: item.text,
          titleTextStyle: _itemsSelected.contains(item) ? Theme.of(context).textTheme.subtitle1.fs18.copyWith(color: Theme.of(context).accentColor) : null,
          leading: item.precedingIcon != null
              ? Icon(item.precedingIcon, color: trailing != null ? Theme.of(context).accentColor : Theme.of(context).hintColor)
              : null,
          trailing: trailing,
