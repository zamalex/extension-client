...
class _SearchFormState extends State<SearchForm> {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      padding: const EdgeInsetsDirectional.only(start: kPaddingM, end: kPaddingM),
      child: Row(
        ...
        children: <Widget>[
          Expanded(
            child: InkWell(
              ...
              child: Card(
                margin: const EdgeInsetsDirectional.only(end: 4, start: 0, top: 0, bottom: 0),
  ...
  Future<void> _showDatePicker(BuildContext context) async {
    final List<PickerItem<DateRangeModel>> _pickerItems = <PickerItem<DateRangeModel>>[];
    final List<PickerItem<DateRangeModel>> _selectedItems = <PickerItem<DateRangeModel>>[];

    for (int _range = 0; _range < kReservationsDateRange; _range++) {
      final DateRangeModel _dateModel = DateRangeModel(_dateRangeToString(_range), _range);
    ...
    if (_returned != null) {
      BlocProvider.of<SearchBloc>(context).add(NewDateRangeSelectedSearchEvent((_returned as DateRangeModel).range));
    }
  }
