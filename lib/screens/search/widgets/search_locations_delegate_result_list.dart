...
class _SearchLocationsDelegateResultListState extends State<SearchLocationsDelegateResultList> {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocBuilder<SearchBloc, SearchState>(
      builder: (BuildContext context, SearchState state) {
        if (state is RefreshSuccessSearchState && state.session.searchType == SearchType.quick) {
          if (state.session.isLoading) {
            ...
            if (q.length >= kMinimalNameQueryLength) {
              ...
              return ListView.builder(
                padding: const EdgeInsets.only(left: kPaddingM, right: kPaddingM, top: 5),
