...
class _FavoritesScreenState extends State<FavoritesScreen> {
  ...
  Widget _showFavorites() {
    ...
    if (_favorites.isNotNullOrEmpty) {
      return AnimatedList(
        ...
        itemBuilder: (BuildContext context, int index, Animation<double> animation) => _buildItem(_favorites[index], animation),
      );
    }
  ...
  void _clearList() {
    for (int i = _favorites.length - 1; i >= 0; i--) {
      final AnimatedListRemovedItemBuilder builder = (BuildContext context, Animation<double> animation) => Container();
  ...
  void _removeFromList(LocationModel location) {
    ...
    // deleted.
    final AnimatedListRemovedItemBuilder builder = (BuildContext context, Animation<double> animation) => _buildItem(location, animation);
