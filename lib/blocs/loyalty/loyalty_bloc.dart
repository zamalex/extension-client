...
import 'package:salon/data/repositories/loyalty_repository.dart';
...
class LoyaltyBloc extends Bloc<LoyaltyEvent, LoyaltyState> {
  ...
  Stream<LoyaltyState> _mapGetVoucherLoyaltyEventToState(VoucherRequestedLoyaltyEvent event) async* {
    yield LoadVoucherInProgressLoyaltyState();

    final VoucherModel voucher = await const LoyaltyRepository().getVoucher(event.voucherId);

    yield LoadVoucherSuccessLoyaltyState(voucher);
  }

  Stream<LoyaltyState> _mapGetVouchersLoyaltyEventToState(VouchersRequestedLoyaltyEvent event) async* {
    yield LoadVouchersInProgressLoyaltyState();

    final List<VoucherModel> vouchers = await const LoyaltyRepository().getVouchers();
