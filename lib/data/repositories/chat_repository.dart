...
class ChatRepository {
  ...
  Future<List<InboxModel>> getInbox() async {
    ...
    final List<dynamic> _ratings = rawData.data['inbox'] as List<dynamic> ?? <dynamic>[];

    return _ratings.map<InboxModel>((dynamic json) => InboxModel.fromJson(json as Map<String, dynamic>)).toList();
  }

  Future<ChatSessionModel> getChatSession({int sessionId}) async {
    final DataResponseModel rawData = await dataProvider.get('messages_' + sessionId.toString());

    return ChatSessionModel.fromJson(rawData.data['session'] as Map<String, dynamic>);
  }
}
