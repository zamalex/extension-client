...
class AppointmentRepository {
  ...
  Future<List<AppointmentModel>> getAppointments() async {
    ...
    final List<dynamic> _appointments = rawData.data['appointments'] as List<dynamic> ?? <dynamic>[];

    return _appointments.map<AppointmentModel>((dynamic json) => AppointmentModel.fromJson(json as Map<String, dynamic>)).toList();
  }

  Future<List<TimetableModel>> getTimetable({int locationId, int duration}) async {
    ...
    for (int days = 0; days < kReservationsDateRange; days++) {
      ...
      for (int t = openingTime; t <= closingTime; t++) {
        final DateTime dateTimeCandidate = DateTime(dateTime.year, dateTime.month, dateTime.day, t, 0, 0);
        if (dateTimeCandidate.millisecondsSinceEpoch > now.millisecondsSinceEpoch && rng.nextInt(5) > 1) {
          _times.add(DateTime(dateTime.year, dateTime.month, dateTime.day, t, 0, 0).millisecondsSinceEpoch);
        }
      }

      timetables.add(TimetableModel.fromJson(<String, dynamic>{
        'date': DateFormat('yyyy-MM-dd', 'en').format(now.add(Duration(days: days))),
        'times': _times,
      }));
    }
